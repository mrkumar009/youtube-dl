...
class YoutubeIE(YoutubeBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if video_description:
            mobj = re.search(r'(?s)(?:Provided to YouTube by (?P<publisher>[^\n]+)\n+(?P<track>[^·\n]+)·(?P<artist>[^\n]+)\n+(?P<album>[^\n]+)(?:.+?(?P<copyright>℗\s*(?P<release_year>\d{4})?\s*[^\n]+))?(?:.+?Released on\s*:\s*(?P<release_date>\d{4}-\d{2}-\d{2}))?(.+?\nArtist\s*:\s*(?P<clean_artist>[^\n]+))?.+\nAuto-generated by YouTube\.\s*$)', video_description)
            ...
            if mobj:
                ...
                if track == album:
                    try:
                        if track.endswith(')'):
                            track, album = track.split('(From "')
                            album = album[:-2]
                    except ValueError:
                        pass
                info.update({
                    'album': album,
                    'artist': mobj.group('clean_artist') or artists,
                    'album_artist': artists_list[0] or None,
                    'track': track.strip(),
                    'publisher': mobj.group('publisher').strip(),
                    'copyright': mobj.group('copyright').strip(),
